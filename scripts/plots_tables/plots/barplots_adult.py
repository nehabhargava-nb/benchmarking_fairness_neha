...
adult_csv_folder = './scripts/plots_tables/csvs/processed_csvs/'
out_plots_folder = './scripts/plots_tables/plots/adult_barplots/'
...
mlp_csv = os.path.join(adult_csv_folder, 'adult-mlp-mean_over_seed_max_over_hp.csv')
laftar_csv = pd.read_csv(os.path.join(adult_csv_folder, 'adult-laftr-mean_over_seed_max_over_hp.csv'))
cfair_mod_csv = pd.read_csv(os.path.join(adult_csv_folder, 'adult-cfair-mean_over_seed_max_over_hp.csv'))
ffvae_csv = os.path.join(adult_csv_folder, 'adult-ffvae-mean_over_seed_max_over_hp.csv')
...
for j in range(7):
    ...
    for k in range(8):
      for l in range(7):
        tab = csvs[k]
        if l == 0:
          first_val = parse_csv(tab, table_type, clr_ratio, sensattr, mean_metric_names, 1)
          second_val = parse_csv(tab, table_type, clr_ratio, sensattr, mean_metric_names, 2)
          avg_val = (first_val + second_val)/2
          val.append(avg_val)
        elif l == 6:
          first_val = parse_csv(tab, table_type, clr_ratio, sensattr, mean_metric_names, 4)
          second_val = parse_csv(tab, table_type, clr_ratio, sensattr, mean_metric_names, 5)
          avg_val = (first_val + second_val)/2
          val.append(avg_val)
        else:
          val.append(parse_csv(tab, table_type, clr_ratio, sensattr, mean_metric_names, l))
    ...
    rects2 = ax.bar(x - 2*width/6, values[1], width/6, yerr=std_values[1], ecolor='gray', capsize=3, label='Laftr-DP', color=clr[1])
    rects3 = ax.bar(x - 1*width/6, values[2], width/6, yerr=std_values[2], ecolor='gray', capsize=3, label='Laftr-EqOpp0', color=clr[2])
    rects4 = ax.bar(x,             values[3], width/6, yerr=std_values[3], ecolor='gray', capsize=3, label='Laftr-EqOpp1', color=clr[3])
    rects5 = ax.bar(x + 1*width/6, values[4], width/6, yerr=std_values[4], ecolor='gray', capsize=3, label='Laftr-EqOdd', color=clr[4])
    rects6 = ax.bar(x + 2*width/6, values[5], width/6, yerr=std_values[5], ecolor='gray', capsize=3, label='Cfair', color=clr[5])
    rects7 = ax.bar(x + 3*width/6, values[6], width/6, yerr=std_values[6], ecolor='gray', capsize=3, label='Cfair-EO', color=clr[6])
    ...
    if j<=5:
      label_name = "Age: (u-elg, u-inelg) = " + clr_ratios[j]
    else:
      label_name = "Sex: (u-elg, u-inelg) = " + "(0.15, 0.38)"
