...
def get_adult_data(target, sensitive, clr_ratio):
    ...
    # Changed by Neha for debugger to run
    ...
    # Added by Neha to remedy the empty array issue
    min_num = int(min(priv_elg, priv_inelg, unpriv_elg, unpriv_inelg))
    #if min_num == 0:
    #    array_minimum = [priv_elg, priv_inelg, unpriv_elg, unpriv_inelg]
    #
    #min_num = int(min(i for i in array_minimum if i > 0))
    # min_num = 1
    # Commented by Neha to remedy the empty array issue
    #min_num = int(min(priv_elg, priv_inelg, unpriv_elg, unpriv_inelg))
